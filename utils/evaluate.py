...
@torch.no_grad()
def get_predictions(_iScan, return_features = False):

    # Make predictions on a dataset with neighbors
    _iScan.model.eval()

    predictions = [[] for _ in range(_iScan.config[_iScan.step]['num_heads'])]
    probs = [[] for _ in range(_iScan.config[_iScan.step]['num_heads'])]
    targets, neighbors = [], []

    if return_features:
        ft_dim = _iScan.model.cluster_head[0].in_features
        features = torch.zeros((len(_iScan.valid_loader.sampler), ft_dim)).cuda()
    
    ptr = 0
    for batch in _iScan.valid_loader:
        images = batch['image'].cuda(non_blocking = True)
        bs = images.shape[0]
        res = _iScan.model(images, forward_pass = 'return_all')
        output = res['output']
        if return_features:
            features[ptr: ptr+bs] = res['features']
            ptr += bs
        for i, output_i in enumerate(output):   # i is head_number (from _iScan.config[_iScan.step]['num_heads'])
            predictions[i].append(torch.argmax(output_i, dim = 1))
            probs[i].append(F.softmax(output_i, dim = 1))
        targets.append(batch['target'])
        if _iScan.step == 'S2':
            neighbors.append(batch['neighborSet'])
